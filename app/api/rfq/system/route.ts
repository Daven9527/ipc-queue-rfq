...
const INDEX_KEY = "rfq:system:ids";
const HASH_KEY = (rfqNo: string) => `rfq:system:${rfqNo}`;

// 生成下一個 System RFQ ID (格式: RFQ(S)-000, RFQ(S)-001, ...)
async function generateNextRfqNo(): Promise<string> {
  ...
  return `RFQ(S)-${nextNum.toString().padStart(3, "0")}`;
}
...
export async function POST(request: Request) {
  try {
    ...
    console.error("create rfq system failed", error);
